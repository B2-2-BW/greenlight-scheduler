...
public class WaitingToEnteredScheduler extends BaseScheduler {
    ...
    protected void registerScheduler() {
        ...
        scheduledTask = scheduler.scheduleWithFixedDelay(() -> {
            if (shouldStop()){
                ...
                log.info("[WAITING_TO_READY] Scheduler stopping");
                return;
            }
            try {
                roomService.processQueue();
            } catch (Exception e) {
                log.error("[WAITING_TO_READY] Scheduler encountered an error", e);
            }
        }, 0, 5, TimeUnit.SECONDS);

        log.info("[WAITING_TO_READY] Scheduler started");
    }
}